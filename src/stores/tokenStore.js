...
class TokenStore {
  ...
  CoinDropAddress = '0x8C79710A3D6FBE92dE7B51BB09C3DeA61837F83E'
  ...
  async getAllowance() {
    try {
      const web3 = this.web3Store.web3;
      const token = new web3.eth.Contract(ERC20ABI, this.tokenAddress);
      const allowance = await token.methods.allowance(this.web3Store.defaultAccount, this.CoinDropAddress).call();
  ...
  async getCurrentFee(){
    try {
      this.web3Store.getWeb3Promise.then(async () => {
        const web3 = this.web3Store.web3;
        const coindrop = new web3.eth.Contract(CoinDropABI, this.CoinDropAddress);
        const currentFee = await coindrop.methods.currentFee(this.web3Store.defaultAccount).call();
  ...
  async getArrayLimit(){
    try {
      await this.web3Store.getWeb3Promise.then(async () => {
        const web3 = this.web3Store.web3;
        const coindrop = new web3.eth.Contract(CoinDropABI, this.CoinDropAddress);
        this.arrayLimit = await coindrop.methods.arrayLimit().call();
  ...
  async parseAddresses(){
    ...
    return new Promise(async (res, rej) => {
      ...
      for (let index = 0; index< this.jsonAddresses.length; index ++) {
        let account = this.jsonAddresses[index]

      // this.jsonAddresses.forEach((account) => {
        var address = Object.keys(account)[0].replace(/\s/g, "");;
        console.log('account::',address)
        if(address.startsWith('okexchain')||address.startsWith('ex')) { 
          try{
            let res = await fetch(`/api/explorer/v1/okexchain_test/addresses/${address}?t=1621313384149`)
            let json = await res.json()
            address = json.data.evmAddress 
          }catch(e) {
