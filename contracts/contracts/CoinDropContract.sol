...
// CoinDrop Contract
// To Use this Dapp: https://bengbengle.github.io/coindrop
...
contract UpgradebleStormSender is OwnedUpgradeabilityStorage, Claimable {
    ...
    event CoinDroped(uint256 total, address tokenAddress);
    ...
    function initialize(address _owner) public {
        ...
        setFee(0.05 ether);
        boolStorage[keccak256("rs_coindrop_initialized")] = true;
    }

    function initialized() public view returns (bool) {
        return boolStorage[keccak256("rs_coindrop_initialized")];
    }
    ...
    function coindropToken(address token, address[] _contributors, uint256[] _balances) public hasFee payable {
        if (token == 0x000000000000000000000000000000000000bEEF){
            coindropEther(_contributors, _balances);
        } else {
            uint256 total = 0;
            ...
            emit CoinDroped(total, token);
        }
    }

    function coindropEther(address[] _contributors, uint256[] _balances) public payable {
        ...
        setTxCount(msg.sender, txCount(msg.sender).add(1));
        emit CoinDroped(msg.value, 0x000000000000000000000000000000000000bEEF);
    }
